...
void add()                                                                //整数加法
{
    ...
    while(iA<=sizeA)
    {
        ...
        if(numA[iA]>9 || numA[iA]<0)
        {
            printf("WRONG INPUT or NOT SUPPORT!Quit....\n");
            fout<<"WRONG INPUT or NOT SUPPORT!"<<endl;
            goto l1;
        }
        iA++;
    }
...
void minus_num()                                                   //整数减法
{
    ...
    sizeB=(int)(strlen(numb));
    //数据输入完成
    if(sizeA<sizeB || (numa[0]<numb[0] && sizeA==sizeB))   //被减数永远为长度最长的数
    {
        while(iA<=sizeB)
        {
...
void matrix_muti()        //矩阵乘法
{
          ...
          if(n1!=n2)     //输入数据错误
        {
            ...
            fout<<"WRONG DIMENSION!"<<endl<<endl<<endl;
            goto l1;
        }
        else
        {
